...
class GUI:
    ...
    def __init__(self, root=None):
        ...
        self.dark_mode = self.theme_manager.theme_var
        self.theme_manager.apply_theme()
            
        # Database management tab will be added in create_tabs
        self.root.title("Database Creator & Management System")
        self.root.geometry("1000x650")  # Slightly taller to accommodate credits
        ...
        credits_frame = ttk.Frame(self.root)
        credits_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=15, pady=(5, 10))
        
        # Add version and credits info
        version_label = ttk.Label(
            credits_frame,
            text="Database Creator & Management System - v1.0.0",
            anchor=tk.W,
            font=("Arial", 8)
        )
    ...
    def create_menu(self):
        ...
        # Options menu (for theme and settings)
    ...
    def create_db_connection_frame(self):
        ...
        ttk.Label(
            db_frame,
            text="Current Database:").grid(row=0,
            column=0,
            padx=5,
            pady=5)
    ...
    def create_query_tab(self):
        ...
        ttk.Label(
            query_frame,
            text="SQL Query:").pack(anchor=tk.W,
            padx=5,
            pady=5)
        ...
        x_scroll = ttk.Scrollbar(
            results_frame, orient=\
                tk.HORIZONTAL, command=self.results_tree.xview
        )
    ...
    def create_data_tab(self):
        ...
        ttk.Label(
            selection_frame,
            text="Select Table:").pack(side=tk.LEFT,
            padx=5)
        ...
        self.data_table_combo.pack(
            side=tk.LEFT,
            padx=5,
            fill=tk.X,
            expand=True)
        self.data_table_combo.bind(
            "<<ComboboxSelected>>",
    ...
    def create_templates_tab(self):
        ...
        ttk.Button(
            left_frame, text=\
                "Apply Template", command=self.apply_selected_template
        ).pack(pady=5)
            ...
            if location_var.get() == "default":
                ...
                if not file_path:
                    messagebox.showerror(
                        "Error",
                        "Please specify a custom location")
                    return
    ...
    def load_table_data(self, table_name):
        ...
        try:
            ...
            # Get table data
            results = \
                      self.db_manager.execute_query(f"SELECT * FROM {table_name}")
    ...
    def apply_template_dialog(self):
        ...
        ttk.Button(
            btn_frame,
            text="Apply",
            command=apply).pack(side=tk.LEFT,
            padx=5)
        ttk.Button(btn_frame, text="Cancel", command=dialog.destroy).pack(
            side=tk.LEFT, padx=5
        )
